import { ZodTypeAny, optional } from "zod"
import { JzodElement } from "./JzodInterface"

const a: null = null;

export const zodToJzod = (
	zod: ZodTypeAny,
	identifier: string,
): JzodElement => {
    ...
    case 'ZodArray': {
      ...
      const propertiesJzodSchema = properties.map(([key, value]) => {
        ...
        const { typeName: nextZodNodeTypeName } = nextZodNode._def
				...
				const isNullable = nextZodNodeTypeName === 'ZodNullable' || nextZodNode.isNullable()
        console.log(
          "zodToJzod",
          typeName,
          key,
        ...
        if (isOptional || (propertyJzodSchema as any)["optional"] != undefined) {
          return [key,{...propertyJzodSchema,optional:isOptional}]
        } else {
          return [key, propertyJzodSchema]
        }
      });
    ...
    case 'ZodNullable': {
      ...
      console.log("zodToJzod converting ZodUnion",JSON.stringify(zod));
			...
			const jzodUnionElements: JzodElement[] = zod._def.options.map((option:ZodTypeAny) => zodToJzod(option, identifier))
      return ({ type: "union", definition: jzodUnionElements})
		}
		case 'ZodDiscriminatedUnion': {
      console.warn("zodToJzod: Zod discriminated unions are converted to unions in Jzod, which are converted back as plain unions in Zod, not discriminated unions as the original Zod Schema.",JSON.stringify(zod));
      
      const jzodUnionElements: JzodElement[] = [...zod._def.options.values()].map((option:ZodTypeAny) => zodToJzod(option, identifier))
      return (zod._def.discriminator?{ type: "union", discriminant: zod._def.discriminator, definition: jzodUnionElements}:{ type: "union", definition: jzodUnionElements})
		}
		case 'ZodEffects': {
